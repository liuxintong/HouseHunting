/* the file name neighborhood_market_tracker.tsv must be replaced with a real absolute file path */

USE `housing`;

CREATE OR REPLACE TABLE `housing`.`neighborhood_market_tracker` (
    period_begin DATE,
    period_end DATE,
    period_duration INT,
...
SELECT * FROM neighborhood_market_tracker LIMIT 10;

LOAD DATA LOW_PRIORITY LOCAL INFILE 'neighborhood_market_tracker.tsv'
INTO TABLE `housing`.`neighborhood_market_tracker`
CHARACTER SET ASCII
...
CREATE OR REPLACE INDEX `period_begin` ON `housing`.`neighborhood_market_tracker` (`period_begin`);
CREATE OR REPLACE INDEX `period_end` ON `housing`.`neighborhood_market_tracker` (`period_end`);
CREATE OR REPLACE INDEX `period_duration` ON `housing`.`neighborhood_market_tracker` (`period_duration`);
CREATE OR REPLACE INDEX `region_type` ON `housing`.`neighborhood_market_tracker` (`region_type`);
CREATE OR REPLACE INDEX `region_type_id` ON `housing`.`neighborhood_market_tracker` (`region_type_id`);
CREATE OR REPLACE INDEX `table_id` ON `housing`.`neighborhood_market_tracker` (`table_id`);
CREATE OR REPLACE INDEX `is_seasonally_adjusted` ON `housing`.`neighborhood_market_tracker` (`is_seasonally_adjusted`);
CREATE OR REPLACE INDEX `region` ON `housing`.`neighborhood_market_tracker` (`region`);
CREATE OR REPLACE INDEX `city` ON `housing`.`neighborhood_market_tracker` (`city`);
CREATE OR REPLACE INDEX `state` ON `housing`.`neighborhood_market_tracker` (`state`);
CREATE OR REPLACE INDEX `state_code` ON `housing`.`neighborhood_market_tracker` (`state_code`);
CREATE OR REPLACE INDEX `property_type` ON `housing`.`neighborhood_market_tracker` (`property_type`);
CREATE OR REPLACE INDEX `property_type_id` ON `housing`.`neighborhood_market_tracker` (`property_type_id`);
CREATE OR REPLACE INDEX `parent_metro_region` ON `housing`.`neighborhood_market_tracker` (`parent_metro_region`);
CREATE OR REPLACE INDEX `parent_metro_region_metro_code` ON `housing`.`neighborhood_market_tracker` (`parent_metro_region_metro_code`);
CREATE OR REPLACE INDEX `last_updated` ON `housing`.`neighborhood_market_tracker` (`last_updated`);
