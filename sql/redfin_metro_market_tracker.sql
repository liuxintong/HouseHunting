/* the file name redfin_metro_market_tracker.tsv must be replaced with a real absolute file path */

USE `housing`;

CREATE OR REPLACE TABLE `housing`.`redfin_metro_market_tracker` (
    period_begin DATE,
    period_end DATE,
    period_duration INT,
...
SELECT * FROM redfin_metro_market_tracker LIMIT 10;

LOAD DATA LOW_PRIORITY LOCAL INFILE 'redfin_metro_market_tracker.tsv'
INTO TABLE `housing`.`redfin_metro_market_tracker`
CHARACTER SET ASCII
